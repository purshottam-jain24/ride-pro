...
<html>
  <head>
    ...
    <style>
      body { padding: 0; margin: 0; background: #e8eef4; }
      #map { width: 100vw; height: 100vh; background: #e8eef4; }
  ...
  <body>
    ...
    <script>
      ...
      document.addEventListener('message', function(event) { try { updateMap(JSON.parse(event.data)); } catch(e){} });
      window.addEventListener('message', function(event) { try { updateMap(JSON.parse(event.data)); } catch(e){} });
    </script>
  </body>
</html>
`;
...
export default function MapComponent({ style, pickup, dropoff, routeCoordinates, driverLocation }: MapComponentProps) {
  ...
  useEffect(() => {
    if (webviewRef.current) {
      const payload = JSON.stringify({ pickup, dropoff, routeCoordinates, driverLocation });
      webviewRef.current.injectJavaScript(`
        if (typeof updateMap === 'function') { updateMap(${payload}); }
        true;
      `);
    }
